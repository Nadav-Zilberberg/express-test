...
res.cookie = function (name, value, options) {
  ...
  if (opts.encrypted) {
    if (!opts.encryptionOptions || !opts.encryptionOptions.secret) {
      throw new Error('encryptionOptions.secret is required for encrypted cookies');
    }
    const algorithm = opts.encryptionOptions.algorithm || 'aes-256-gcm';
    const ivLength = opts.encryptionOptions.ivLength || 12;
    const secret = opts.encryptionOptions.secret;

    // Encryption logic here (implementation omitted for brevity)
    val = 'e:' + encryptedValue;
  }
