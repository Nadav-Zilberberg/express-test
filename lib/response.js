 ...
 *    - `signed`   sign the cookie
 *    - `path`     defaults to "/"
 *    - `encrypted`  encrypt the cookie
 *    - `encryptionOptions`  options for encryption, including:
 *        - `secret` the encryption secret
 *        - `algorithm` the encryption algorithm (default: 'aes-256-gcm')
 *        - `ivLength` the length of the initialization vector (default: 12)
 *
 * Examples:
 *
 *    // "Remember Me" for 15 minutes
...
res.cookie = function (name, value, options) {
  ...
  if (opts.encrypted) {
    const iv = crypto.randomBytes(opts.encryptionOptions.ivLength || 12);
    const cipher = crypto.createCipheriv(opts.encryptionOptions.algorithm || 'aes-256-gcm', opts.encryptionOptions.secret, iv);
    let encrypted = cipher.update(val, 'utf8', 'base64');
    encrypted += cipher.final('base64');
    val = `e:${iv.toString('base64')}.${encrypted}`;
  }
  if (signed) {
