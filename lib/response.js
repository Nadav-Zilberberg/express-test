...
res.cookie = function (name, value, options) {
  ...
  if (opts.encrypted && !secret) {
    throw new Error('cookieParser("secret") required for encrypted cookies');
  }
  ...
  if (opts.encrypted) {
    const algorithm = opts.encryptionOptions?.algorithm || 'aes-256-gcm';
    const secret = opts.encryptionOptions?.secret;
    const ivLength = opts.encryptionOptions?.ivLength || 12; // Default IV length for GCM
